...
void msg_buffer_send(msg_buffer *builder, struct dpu_set_t dpu_set, bool async)
{
    if(async) {
        DPU_ASSERT(dpu_broadcast_to(dpu_set, DPU_MRAM_HEAP_POINTER_NAME, MESSAGE_BUFFER_OFFSET, builder->buffer,
                                builder->header_ptr->total_size, DPU_XFER_ASYNC));
    } else {
        DPU_ASSERT(dpu_broadcast_to(dpu_set, DPU_MRAM_HEAP_POINTER_NAME, MESSAGE_BUFFER_OFFSET, builder->buffer,
...
void msg_buffer_recv(struct dpu_set_t dpu_set, void **dst, uint32_t size_per_dpu, bool async)
{
    ...
    if(async) {
        DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_FROM_DPU, DPU_MRAM_HEAP_POINTER_NAME, RESULT_BUFFER_OFFSET, size_per_dpu, DPU_XFER_ASYNC));
    } else {
